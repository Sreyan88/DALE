...
class SketchExtractor:
    def __init__(self, model='yake'):
        assert model in ['yake', 'bert','jieba', 'lexlms'], '`model` only support `yake`, `bert` or `jieba`'
        self.model = model
        self.mask = '<mask>'
        self.sep = ' '
        ...
        if model == 'bert': # for English
            self.extractor = AspectKeyBERT(model='intfloat/e5-large-v2') # paraphrase-MiniLM-L3-v2 (the fastest LM) all-MiniLM-L6-v2
    ...
    def get_kws(self, s, min_ngrams=1, max_ngram=3, top=10, aspect_keywords=None, use_aspect_as_doc_embedding=False, content=None, mentions=None, tok_type="word"):
        if self.model == 'yake':
            self.extractor = yake.KeywordExtractor(n=max_ngram,top=top, windowsSize=1)
            kws_pairs = self.extractor.extract_keywords(s)
        if self.model == 'bert' or self.model== 'lexlms':
            kws_pairs = self.extractor.extract_aspect_keywords(s,top_n=top, keyphrase_ngram_range=(min_ngrams, max_ngram),
                                            aspect_keywords=aspect_keywords,
                                            use_aspect_as_doc_embedding=use_aspect_as_doc_embedding,content=content, mentions=mentions, tok_type=tok_type)
        if self.model == 'jieba':
    ...
    def get_sketch(self, s, max_ngram=3, top=10, aspect_keywords=None, use_aspect_as_doc_embedding=False, template=4):
        _, kws = self.get_kws(s, max_ngram, top, aspect_keywords=aspect_keywords, use_aspect_as_doc_embedding=use_aspect_as_doc_embedding)
