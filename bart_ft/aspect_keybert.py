...
def nltk_whitespace_tokenizer(text):
        ...
        try:
            ...
            if candidates is None:
                if vectorizer:
                    ...
                    if content is None:
                        # if tok_type =="word":
                        #     txt_tokenizer = word_tokenize
                        # else:
                        #     print("whitespace tok")
                        #     txt_tokenizer = nltk_whitespace_tokenizer
                ...
                # if mentions is not None and tok_type == "space":
                #     print("Inside filtering candidates")
                #     final_candidates = []
                ...
                candidates = ["query: " + c for c in candidates if not there_is_punc(c)]
            doc = "passage: " + doc
            # Extract Embeddings
            ...
            if aspect_keywords is not None and use_aspect_as_doc_embedding == False:
                aspect_input = "passage: " + " ".join(aspect_keywords)
                aspect_embeddings = self.model.embed([aspect_input])
            ...
            if use_mmr:
                ...
                keywords = [
                    (candidates[index].replace('query: ', ''), round(float(distances[0][index]), 4))
